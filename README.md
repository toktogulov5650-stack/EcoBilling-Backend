...
Созданы архитектурный каркас, примитивы `Error`/`Result`, исполняемые проверки графа зависимостей, Identity с PostgreSQL persistence и профиль Resident с отдельной schema, repository и сценарием получения по `UserId`. HTTP-аутентификация, полное создание жителя, Accounts, Addresses, токены, тарифы и расчёт начислений будут проектироваться отдельными этапами.
